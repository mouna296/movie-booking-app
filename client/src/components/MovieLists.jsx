...
const MovieLists = ({ movies, search, handleDelete,handleUpdate }) => {
	...
	return !!moviesList.length ? (
		<div className="grid grid-cols-1 gap-4 rounded-md bg-slate-100 p-4 drop-shadow-md lg:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4 min-[1920px]:grid-cols-5">
				...
				return (
					<div key={index} className="flex min-w-fit flex-grow rounded-md bg-white drop-shadow-md">
						...
						<div className="flex flex-grow flex-col justify-between p-2">
							...
							<button
								className="flex w-fit items-center gap-1 self-end rounded-md bg-gradient-to-br from-yellow-700 to-yellow-600 py-1 pl-2 pr-1.5 text-sm font-medium text-white hover:from-red-600 hover:to-rose-500"
